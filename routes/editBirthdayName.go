...
func EditBirthdayName(coll *dynamodb.DynamoDB, table string, name string, mobile int64, body string) events.LambdaFunctionURLResponse {
	...
	if !utils.ValidName(name) || !utils.ValidName(body) {
		errMsg.StatusCode = http.StatusBadRequest
